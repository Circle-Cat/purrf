...
| [azuread_application_federated_identity_credential.azure_wif_cf](https://registry.terraform.io/providers/hashicorp/azuread/latest/docs/resources/application_federated_identity_credential) | resource |
...
| [google_storage_bucket.resumes](https://registry.terraform.io/providers/hashicorp/google/latest/docs/resources/storage_bucket) | resource |
...
| <a name="input_ingress_class_name"></a> [ingress\_class\_name](#input\_ingress\_class\_name) | Kubernetes ingress class name for the app ingress | `string` | `"cloudflare-tunnel"` | no |
| <a name="input_jira_password"></a> [jira\_password](#input\_jira\_password) | n/a | `string` | n/a | yes |
| <a name="input_k8s_cluster_location"></a> [k8s\_cluster\_location](#input\_k8s\_cluster\_location) | GKE cluster region or zone (used for Azure Workload Identity Federation issuer URL) | `string` | `"us-west1"` | no |
| <a name="input_k8s_cluster_name"></a> [k8s\_cluster\_name](#input\_k8s\_cluster\_name) | GKE cluster name (used for Azure Workload Identity Federation issuer URL) | `string` | `"dev"` | no |
| <a name="input_k8s_project_id"></a> [k8s\_project\_id](#input\_k8s\_project\_id) | GCP project ID that hosts the GKE cluster (used for Workload Identity pool binding) | `string` | `"circlecat"` | no |
...
| <a name="input_tailscale_proxy"></a> [tailscale\_proxy](#input\_tailscale\_proxy) | Tailscale outbound proxy URL. If empty, TAILSCALE\_PROXY is not set. | `string` | `""` | no |
...
| <a name="output_resume_bucket_name"></a> [resume\_bucket\_name](#output\_resume\_bucket\_name) | n/a |
