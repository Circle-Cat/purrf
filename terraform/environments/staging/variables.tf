...
variable "gmail_sender_recruiting" {
  description = "From address for recruiting email — a Send-As alias of purrf@circlecat.org (recruiting-staging@circlecat.org), so a mail from this environment is recognisable as one and can never claim the real recruiting identity."
  type        = string
}
