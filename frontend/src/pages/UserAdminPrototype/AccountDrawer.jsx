 ...
 * on them. The whole drawer sits behind user.admin, so there is no second
 * permission gate inside it; permissions themselves are a link out to the
 * permission-management page rather than a section here.
 *
 * The link is shown to every user.admin holder regardless of whether they can
 * manage permissions. It is only a link — the destination enforces its own
 * gate, and duplicating that check here would put the same rule in two places.
 *
 * @param {{user: object|null, request: object|null, onClose: Function,
        ...
        <div className="space-y-5 px-4 pb-6">
          ...
          {request && request.status === "pending" && (
            <section className="space-y-3 rounded-md border border-amber-300 bg-amber-50 p-3">
              ...
              <p className="text-xs text-amber-800">
                Approving applies the block immediately, with the same
                consequences shown when it was raised.
              </p>
            </section>
          )}
          ...
          <Section title="Account state">
            ...
            {!user.isActive && (
              <p className="text-sm text-slate-700">
                Deactivated {user.deactivatedAt} by{" "}
                {actorName(user.deactivatedBy)}
          ...
          <Section title="Sign-in methods">
            ...
            <p className="pt-1 text-xs text-slate-500">
              Read-only. Removing or relinking a sign-in method is not part of
              this console.
            </p>
          </Section>
          ...
          <Section title="Actions">
            ...
            {isSelf && (
              <p className="text-xs text-slate-500">
                You cannot deactivate or block yourself — it would lock you out
                of the console needed to undo it.
              </p>
            )}
          </Section>
          ...
          <Section title="Permissions">
            ...
            <p className="text-xs text-slate-500">
              Opens the permission-management page with this person selected.
              That page enforces its own gate.
            </p>
          </Section>
        </div>
      </SheetContent>
