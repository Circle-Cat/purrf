 ...
 * MeetingLogTable
 *
 * The body of the console's `MeetingLogDialog` — same columns, same
 * three-state status, same name-substituted attendance tags, same batch Edit
 * mode with its mutual-exclusion rules and before/after confirmation — lifted
 * out of the dialog so the pair page can show it inline. It is copied here
 * rather than imported only because the prototype has no backend; the real
 * change is to extract this from the dialog, not to write a second one.
 *
 * Only a v2 round can be edited, as today.
 *
 * @param {{
            ...
            <tbody>
              {meetings.map((meeting, index) => {
                ...
                const effectiveFields =
                  isEditing && !isChecked ? getEffectiveFields(meeting) : null;
                return (
                  <tr
                        ...
                        {isEditing && (
                          <Checkbox
                            ...
                            onCheckedChange={(checked) =>
                              togglePendingDelete(meeting.meetingId, checked)
                            }
                          />
                        )}
                        {index + 1}
