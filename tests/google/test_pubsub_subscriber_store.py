...
class TestPubsubSubscriberStore(unittest.TestCase):
    def setUp(self):
        ...
        self.fake_client.subscription_path.side_effect = (
            self.fake_subscriber.subscription_path
        )
    ...
    def test_pull_messages_expiration_reminder_success(self, mock_renew_subscription):
        message_dict = {"subscription": {"name": "subscriptions/test-id"}}
        attr = {"ce-type": "google.workspace.events.subscription.v1.expirationReminder"}
        ...
        mock_renew_subscription.assert_called_once_with(
            "test_project", "subscriptions/test-id"
        )
        msg.ack.assert_called_once()

    @patch("google.pubsub_subscriber_store.renew_subscription")
    def test_expiration_reminder_missing_subscription_name(
        self, mock_renew_subscription
    ):
    ...
    def test_pull_messages_missing_fields(self, mock_get_ldap_by_id, mock_store):
        ...
        attr = {"ce-type": "google.workspace.chat.message.v1.created"}
        msg_dict = {"message": {"sender": {}, "space": {"name": "spaces/test"}}}
