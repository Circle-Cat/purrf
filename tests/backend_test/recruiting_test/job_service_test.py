...
class TestJobService(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_submit_for_review_records_review_opened(self):
        ...
        record.assert_awaited_once_with(
            ...
            details={"kind": "initial", "reviewerIds": [2], "message": "please"},
        )

    async def test_review_opened_details_actually_resolve_to_the_reviewer(self):
        """The details this service writes must satisfy the real resolver.

        Nothing else ties the two together: the resolver's own test builds
        the payload it wants to read, and an assertion here on a literal key
        would only restate what the code does. Feeding what this service
        actually emits through the real resolver is what fails when either
        side moves -- and a mismatch is silent, opening the review while
        notifying nobody.
        """
        self._two_approvers()
        job = self._job(status=JobStatus.DRAFT)
        ...
        emitted = record.await_args.kwargs
        event = EventEntity(
            subject_type=emitted["subject_type"],
            subject_id=emitted["subject_id"],
            actor_id=emitted["actor_id"],
            event_type=emitted["event_type"],
            details=emitted["details"],
        )

        self.assertEqual(await resolve_recipients(self.session, event), {2})
