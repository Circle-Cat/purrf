...
from backend.historical_data.jira_history_fetcher import (
...
class TestProcessIncrementalIssuesData(TestCase):
    ...
    @patch("backend.historical_data.jira_history_fetcher.JIRA_STATUS_ID_MAP")
...
class TestUpdateIncrementalIssueIndexes(TestCase):
    ...
    @patch(
        "backend.historical_data.jira_history_fetcher.remove_updated_issue_previous_indexes"
    )
...
class TestProcessUpdateJiraIssues(TestCase):
    ...
    def test_coordinates_incremental_update_workflow(
        ...
        with patch(
            "backend.historical_data.jira_history_fetcher.process_incremental_issues_data"
        ) as mock_process:
...
class TestFetchIssuesWithMetadata(TestCase):
    ...
    @patch(
        "backend.historical_data.jira_history_fetcher.JIRA_EXCLUDED_STATUS_ID",
        "excluded_status",
    )
...
class TestFetchAndStoreIssuesForLdap(TestCase):
    ...
    @patch("backend.historical_data.jira_history_fetcher.fetch_issues_with_metadata")
    @patch("backend.historical_data.jira_history_fetcher.store_issues_in_redis")
