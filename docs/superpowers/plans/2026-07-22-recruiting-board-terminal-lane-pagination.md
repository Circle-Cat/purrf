...
### Task 2: Alembic migration — add column, backfill, NOT NULL, index

**Files:**
- Create: `alembic_setup/versions/<rev>_add_stage_entered_at_to_application.py`
- Test: `tests/backend_test/repository_test/stage_entered_at_backfill_test.py` (create) + its `BUILD` entry

**Interfaces:**
- Consumes: `ApplicationEntity.stage_entered_at` from Task 1.
- Produces: the column materialized + backfilled on the DB; index `ix_application_job_stage_entered`.
...
- [ ] **Step 2: Replace `upgrade()`/`downgrade()` with the add-nullable → backfill → set-not-null → index sequence**
...
Create `tests/backend_test/repository_test/stage_entered_at_backfill_test.py`. It seeds rows + activity, runs the **backfill UPDATE statement** (import the SQL as a module-level constant from the migration is impractical; copy the same UPDATE text into the test), and asserts. Structure (DB-backed, uses `DATABASE_URL`; follow `application_repository_test.py`'s session/setup style):

```python
import unittest
# ... async test base used elsewhere in this dir ...

BACKFILL_SQL = """<paste the exact UPDATE from Step 2>"""

class StageEnteredAtBackfillTest(...):
    async def test_manual_move_uses_last_stage_changed_to_current_stage(self):
        # app currently REJECTED, with stage_changed toStage=tech at T1,
        # then stage_changed toStage=rejected at T2 (T2 > T1).
        # After backfill: stage_entered_at == T2.
        ...
        self.assertEqual(app.stage_entered_at, T2)
    ...
    async def test_blacklisted_row_uses_blacklisted_event_time(self):
        ...
        # After backfill: stage_entered_at == TB.
        ...

    async def test_hired_then_swept_to_rejected_does_not_use_hire_time(self):
        # stage_changed toStage=hired at TH, then 'blacklisted' at TS (TS>TH),
        # row now REJECTED. After backfill: stage_entered_at == TS (NOT TH).
        ...

    async def test_auto_landed_row_falls_back_to_created_datetime(self):
        # app REJECTED with only an 'auto_rejected' activity.
        # After backfill: stage_entered_at == created_datetime.
        ...
...
Add a `py_test(name = "stage_entered_at_backfill_test", srcs = [...], ...)` entry to `tests/backend_test/repository_test/BUILD` mirroring the existing `application_repository_test` entry (same deps + `DATABASE_URL` env passing).

- [ ] **Step 4: Run the backfill test to verify it fails**

Run: `DATABASE_URL=$DATABASE_URL bazel test //tests/backend_test/repository_test:stage_entered_at_backfill_test --test_env=DATABASE_URL --test_output=errors`
Expected: FAIL (column/backfill not applied to the test DB yet, or assertions unmet).

- [ ] **Step 5: Apply the migration to the test DB**

Run: `DATABASE_URL=$DATABASE_URL python tools/migrate_db/migrate_db.py`
Expected: migration applies cleanly; `application.stage_entered_at` exists and is backfilled + NOT NULL.
(Confirm `DATABASE_URL` points at the **test** DB before running — never run against staging/prod.)

- [ ] **Step 6: Run the backfill test to verify it passes**
...
```bash
git add alembic_setup/versions/ tests/backend_test/repository_test/stage_entered_at_backfill_test.py \
        tests/backend_test/repository_test/BUILD
git commit -m "[PUR-505] Migrate stage_entered_at column with backfill and index
...
DATABASE_URL=$DATABASE_URL bazel test //tests/backend_test/recruiting_test:board_service_test \
  ...
  //tests/backend_test/repository_test:stage_entered_at_backfill_test \
