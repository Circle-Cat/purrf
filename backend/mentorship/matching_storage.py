...
class MatchingStorage:
    ...
    def write_input(self, run_id: str, matching_input) -> None:
        """Write everything the matcher reads.

        The round's pointer is not moved here. It is what the review screen
        reads, so it waits for ``point_round_at`` once the job has started;
        moving it now would hide the round's last run behind one that may never
        begin.
    ...
    def point_round_at(self, round_id: int, run_id: str) -> None:
        """Make this run the one the review screen finds for its round."""
        self.redis_client.set(
