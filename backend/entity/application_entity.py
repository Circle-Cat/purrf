...
class ApplicationEntity(Base):
    ...
    stage: Mapped[ApplicationStage] = mapped_column(
        Enum(ApplicationStage, name="application_stage_enum", values_callable=lambda obj: [e.value for e in obj]),
        nullable=False,
    )
    form_answers: Mapped[dict | None] = mapped_column(JSONB)
    snapshot: Mapped[dict | None] = mapped_column(JSONB)
    is_viewed: Mapped[bool] = mapped_column(Boolean, default=False, server_default="false", nullable=False)
    previous_application_id: Mapped[int | None] = mapped_column(ForeignKey("application.application_id"))
    rejected_round_id: Mapped[int | None] = mapped_column(ForeignKey("mentorship_round.round_id"))
    rejected_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True))
    created_datetime: Mapped[datetime] = mapped_column(DateTime(timezone=True), server_default=func.now())
