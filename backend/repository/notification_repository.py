...
class NotificationRepository:
    ...
    row carries the email state machine, so deleting it would drop a mail
    that had not gone out yet and erase the record of one that had. Every
    read here is therefore scoped to undismissed rows.

    Email delivery for the new event-based rows is NOT here: it is a state
    machine (PENDING/SENDING/SENT/FAILED/EXPIRED) owned end-to-end by
    ``DeliveryService``, which claims/settles rows with its own inline
    ``update()`` statements rather than through this repository. The
    ``claim_unemailed``/``mark_emailed`` pair that used to live here served
    the old ``email_sent_at``-polling worker (``NotificationEmailWorker``,
    removed) and had no equivalent once that worker was retired.
    """
