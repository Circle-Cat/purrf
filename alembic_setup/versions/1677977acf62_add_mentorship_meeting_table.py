...
def upgrade() -> None:
    ...
    sa.Enum("manual", "google", "legacy", name="meeting_source_enum").create(
        op.get_bind()
    )
    op.create_table(
        "mentorship_meeting",
        ...
        sa.Column(
            "entry_points", postgresql.JSONB(astext_type=sa.Text()), nullable=True
        ),
        ...
        sa.CheckConstraint(
            "source = 'google' OR (meet_link IS NULL AND google_meeting_code IS NULL AND entry_points IS NULL AND absent_user_id IS NULL AND late_user_ids IS NULL AND has_unknown_absent IS NULL AND has_unknown_late IS NULL AND has_insufficient_duration IS NULL AND last_sync_at IS NULL)",
            name=op.f("ck_mentorship_meeting_google_fields"),
        ),
        sa.CheckConstraint(
            "source = 'legacy' OR (start_datetime IS NOT NULL AND end_datetime IS NOT NULL AND end_datetime > start_datetime)",
            name=op.f("ck_mentorship_meeting_times"),
        ),
        sa.ForeignKeyConstraint(
            ["pair_id"],
            ["mentorship_pairs.pair_id"],
            name=op.f("fk_mentorship_meeting_pair_id_mentorship_pairs"),
            ondelete="CASCADE",
        ),
    ...
    op.create_index(
        ...
        ["google_meeting_code"],
        unique=False,
        postgresql_where=sa.text("google_meeting_code IS NOT NULL"),
    )
    op.create_index(
        "ix_mentorship_meeting_pair_start",
        "mentorship_meeting",
        ["pair_id", "start_datetime"],
        unique=False,
    )
    op.create_index(
...
def downgrade() -> None:
    ...
    sa.Enum("manual", "google", "legacy", name="meeting_source_enum").drop(
        op.get_bind()
    )
    # ### end Alembic commands ###
